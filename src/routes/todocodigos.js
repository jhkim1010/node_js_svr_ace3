...
router.get('/:id', async (req, res) => {
    ...
    try {
        ...
        logTableError('todocodigos', 'fetch todocodigo', err, req);
...
router.post('/', async (req, res) => {
    try {
        ...
        logTableError('todocodigos', 'create/update todocodigo (POST)', err, req);
    ...
    try {
        ...
        try {
            ...
            try {
                ...
                // 영향받은 codigos 개수 확인
                const codigosCountQuery = `
                    SELECT COUNT(*) as count
                    FROM codigos c
                    WHERE c.ref_id_todocodigo = :todocodigoId
                `;
                const [codigosCountResult] = await sequelize.query(codigosCountQuery, {
                    replacements: { todocodigoId: id },
                    type: Sequelize.QueryTypes.SELECT
                });
                
                affectedCodigosCount = parseInt(codigosCountResult.count, 10);
            ...
            // 최소한의 요약만 출력
            console.log(`[Todocodigo Update] id=${id} | 변경된 컬럼: ${changedFieldsSummary.length > 0 ? changedFieldsSummary.join(', ') : '없음'} (${changedFieldsSummary.length}개) | 영향받은 Codigos: ${affectedCodigosCount}개`);
        ...
        logTableError('todocodigos', 'update todocodigo', err, req);
        if (!res.headersSent) res.status(400).json({ error: 'Failed to update todocodigo', details: err.message });
    }
}
...
router.delete('/:id', async (req, res) => {
    ...
    try {
        ...
        logTableError('todocodigos', 'delete todocodigo', err, req);
        if (!res.headersSent) res.status(400).json({ error: 'Failed to delete todocodigo', details: err.message });
    }
});
