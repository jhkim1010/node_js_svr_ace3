...
router.post('/', async (req, res) => {
    try {
        ...
        handleInsertUpdateError(err, req, 'Parametros', ['progname', 'pname', 'opcion'], 'parametros');
        res.status(400).json({ 
...
router.put('/:progname/:pname/:opcion', async (req, res) => {
    ...
    try {
        ...
        if (Array.isArray(req.body.data) && req.body.data.length > 0) {
            ...
            // 50개를 넘으면 배치로 나눠서 처리
            const result = await processBatchedArray(req, res, handleArrayData, Parametros, ['progname', 'pname', 'opcion'], 'Parametros');
            await notifyBatchSync(req, Parametros, result);
            return res.status(200).json(result);
        }
