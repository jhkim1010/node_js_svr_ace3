...
router.post('/', async (req, res) => {
    try {
        ...
        handleInsertUpdateError(err, req, 'GastoInfo', ['id_gasto', 'codigo'], 'gasto_info');
        res.status(400).json({ 
...
router.put('/:id_gasto/:codigo', async (req, res) => {
    ...
    try {
        ...
        if (Array.isArray(req.body.data) && req.body.data.length > 0) {
            ...
            // 50개를 넘으면 배치로 나눠서 처리
            const result = await processBatchedArray(req, res, handleArrayData, GastoInfo, ['id_gasto', 'codigo'], 'GastoInfo');
            await notifyBatchSync(req, GastoInfo, result);
            return res.status(200).json(result);
        }
