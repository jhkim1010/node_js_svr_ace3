...
async function getIngresosReport(req) {
    ...
    // 디버깅: 쿼리 실행 조건 확인
    console.log('[Ingresos 보고서] 쿼리 실행 조건:');
    ...
    // 기간이 변경되면 필터를 무시
    let productWhereConditions = [whereClause];
    
    // sucursal 필터 추가 (있을 경우)
    if (sucursalInt !== null && !isNaN(sucursalInt)) {
        productWhereConditions.push(`i.sucursal = ${sucursalInt}`);
    }
    ...
    const productQuery = `
        ...
        GROUP BY i.codigo${includeSucursal ? ', i.sucursal' : ''}
        ORDER BY i.codigo${includeSucursal ? ', i.sucursal' : ''}
    `;
    ...
    if (includeSucursal) {
        ...
        console.log(`   [중요] Product 쿼리는 codigo로만 GROUP BY 처리됨`);
    }
    ...
    try {
        ...
        // 디버깅: 쿼리 결과 상세 로깅
        console.log('[Ingresos 보고서] 쿼리 실행 결과 (상세):');
    ...
    console.log('[Ingresos 보고서] Resumen 필터링 (상세):');
    ...
    console.log('[Ingresos 보고서] 최종 데이터 개수:');
