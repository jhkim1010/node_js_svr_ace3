        ...
        for (let i = 0; i < req.body.data.length; i++) {
            ...
            try {
                ...
                if (availableUniqueKey) {
                    ...
                    console.log(`\n📋 [Vdetalle] Item ${i}: Found unique key (${keyInfo}) - Searching for existing record...`);
                    ...
                    if (existingRecord) {
                        console.log(`   🔍 [Vdetalle] Item ${i}: Found existing record:`, JSON.stringify(existingRecord.toJSON(), null, 2));
                        console.log(`   📝 [Vdetalle] Item ${i}: Attempting UPDATE...`);
                        ...
                        if (count > 0) {
                            const updated = Array.isArray(availableUniqueKey)
                                ? await Model.findOne({ where: whereCondition, transaction })
                                : await Model.findByPk(filteredItem[availableUniqueKey], { transaction });
                            console.log(`   ✅ [Vdetalle] Item ${i}: UPDATE successful`);
                            results.push({ index: i, action: 'updated', data: updated });
                            updatedCount++;
                        } else {
                            console.log(`   ⚠️  [Vdetalle] Item ${i}: UPDATE failed - Switching to INSERT`);
                            const created = await Model.create(filteredItem, { transaction });
                            results.push({ index: i, action: 'created', data: created });
                            createdCount++;
                        }
                    } else {
                        console.log(`   ⚠️  [Vdetalle] Item ${i}: No existing record found - Switching to INSERT`);
                        const created = await Model.create(filteredItem, { transaction });
                        results.push({ index: i, action: 'created', data: created });
                        createdCount++;
                    }
                } else {
                    console.log(`\n📋 [Vdetalle] Item ${i}: No unique key found - Performing INSERT`);
                    const created = await Model.create(filteredItem, { transaction });
                    console.log(`   ✅ [Vdetalle] Item ${i}: INSERT successful`);
                    results.push({ index: i, action: 'created', data: created });
                    createdCount++;
                }
                ...
                console.error(`❌ Vdetalle item ${i} processing failed:`, err.message);
                errors.push({ 
                    index: i, 
                    error: err.message,
            ...
            try {
                ...
                if (operation === 'INSERT' || operation === 'UPDATE' || operation === 'CREATE') {
                    ...
                    if (availableUniqueKey) {
                        const whereCondition = buildWhereCondition(filteredItem, availableUniqueKey);
                        const keyInfo = Array.isArray(availableUniqueKey) ? availableUniqueKey.join(', ') : availableUniqueKey;
                        console.log(`\n📋 [Vdetalle] Item ${i} (${operation}): Found unique key (${keyInfo}) - Searching for existing record...`);
                        ...
                        console.log(`\n📋 [Vdetalle] Item ${i} (${operation}): No unique key found - Performing INSERT`);
                        const created = await Model.create(filteredItem, { transaction });
                        console.log(`   ✅ [Vdetalle] Item ${i}: INSERT successful`);
                        results.push({ index: i, action: 'created', data: created });
                        createdCount++;
